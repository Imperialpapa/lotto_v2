...
class LottoGeneratorLayout(BoxLayout):
    ...
    def init_local_database_connection(self):
        ...
        if self.local_db:
            ...
            self.ids.db_status_label.text = "로컬 데이터베이스: 연결됨 ✅"
            self.load_data_from_local_database()
            self.update_default_round_values()
            self.start_auto_update()
        else:
            self.ids.db_status_label.text = "로컬 데이터베이스: 연결 실패 ❌"

    def load_data_from_local_database(self):
        if not self.local_db_connected: return
    ...
    def query_winning_numbers(self):
        if not self.local_db_connected:
            popup = Popup(title='오류', content=Label(text='로컬 데이터베이스에 연결되지 않았습니다.'), size_hint=(0.8, 0.4))
            popup.open()
            return
    ...
    def start_auto_update(self):
        if not self.local_db_connected:
            return
        self.ids.db_status_label.text = "로컬 데이터베이스: 업데이트 확인 중..."
        updater = LocalDatabaseUpdater(
            database_instance=self.local_db,
    ...
    def update_finished_callback(self, message):
        Clock.schedule_once(lambda dt: self._update_label(message))
        self.load_data_from_local_database() # 업데이트 완료 후 데이터 다시 로드
        self.update_default_round_values() # 기본 회차값도 다시 설정

    def _update_label(self, message):
        self.ids.db_status_label.text = f"로컬 데이터베이스: {message}"

# Kivy 레이아웃 파일 로드
Builder.load_file('lotto.kv')
